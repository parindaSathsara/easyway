...
function ApprovedPartners() {
  ...
  const formSubmit = () => {
    confirmAlert({
      ...
      message: 'Are you sure to activate selling of this partner?',
      buttons: [
        {
          label: 'Yes',
          onClick: () =>{
  ...
  const partnersData = {
    columns: [
      ...
      },
      {
        ...
        field: 'partnerid',
        render: partner =>
          <button className='btn btn-light text-dark' data-toggle="modal" data-target="#approvedPartners" style={{ fontSize: 14 }} value={partner.partnerid} onClick={handleApproveButtonOnClick}>
            <i class="fa fa-file-text" aria-hidden="true" style={{ marginRight: 8 }}></i>Selling</button>
      },

    ],


    rows: approvedPartners.map(partnerApproval => {
      return {
  ...
  const getPartnersByStatus = () => {
    axios.get('/api/partners/getPartnersByStatus/DocumentsApproved').then(res => {

      if (res.data.status == 200) {
        setApprovedPartners(res.data.partners)
      }
      else {
        console.log("NoData")
      }
  ...
  return (
    <>
      <Snackbar
        ref={snackbarRef}
        message="Partner Selling Activated !"
        type={SnackbarType.success}
      />

      <Snackbar
        ref={snackbarRefErr}
        message="Selling Activated Unsuccessfully !"
      ...
      <div class="modal fade" id="approvedPartners" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
          <div class="modal-content">
            ...
            <div class="modal-footer">
              ...
              <button type="button" class="btn btn-dark" onClick={formSubmit}>Activate Selling</button>
            </div>
          </div>
        </div>
      </div>
